...
import (
	...
	"github.com/stockyard-dev/stockyard-barrage/internal/license"
	"github.com/stockyard-dev/stockyard-barrage/internal/server"
	"github.com/stockyard-dev/stockyard-barrage/internal/store"
)

func main() {
	port := getEnv("PORT", "9060")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("BARRAGE_LICENSE_KEY")
	...
	log.Printf("Stockyard Barrage listening on :%s (tier: %s)", port, tier)
