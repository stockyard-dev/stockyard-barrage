package server
import ("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-barrage/internal/store")
...
func(s *Server)routes(){
    ...
    s.mux.HandleFunc("POST /api/scenarios",s.handleCreateScenario)
    s.mux.HandleFunc("GET /api/scenarios/{id}",s.handleGetScenario)
    s.mux.HandleFunc("DELETE /api/scenarios/{id}",s.handleDeleteScenario)
    s.mux.HandleFunc("POST /api/scenarios/{id}/run",s.handleRunScenario)
    s.mux.HandleFunc("GET /api/runs",s.handleListRuns)
    s.mux.HandleFunc("GET /",s.handleUI)
}
func(s *Server)handleHealth(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-barrage"})}  
func(s *Server)handleVersion(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"version":"0.1.0","service":"stockyard-barrage"})}  
