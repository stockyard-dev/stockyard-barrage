...
func Open(d string) (*DB, error) {
	...
	db, err := sql.Open("sqlite", filepath.Join(d, "barrage.db")+"?_journal_mode=WAL&_busy_timeout=5000")
	if err != nil {
		return nil, err
	}
...
func (d *DB) CreateTest(t *Test) error {
	...
	_, err := d.db.Exec(`INSERT INTO tests VALUES(?,?,?,?,?,?,?,?,?)`, t.ID, t.Name, t.URL, t.Method, t.Headers, t.Body, t.Concurrency, t.Requests, t.CreatedAt)
	return err
}
func (d *DB) GetTest(id string) *Test {
	var t Test
	if d.db.QueryRow(`SELECT id,name,url,method,headers,body,concurrency,requests,created_at FROM tests WHERE id=?`, id).Scan(&t.ID, &t.Name, &t.URL, &t.Method, &t.Headers, &t.Body, &t.Concurrency, &t.Requests, &t.CreatedAt) != nil {
		return nil
	}
	d.db.QueryRow(`SELECT COUNT(*) FROM runs WHERE test_id=?`, id).Scan(&t.RunCount)
	return &t
}
func (d *DB) ListTests() []Test {
	rows, _ := d.db.Query(`SELECT id,name,url,method,headers,body,concurrency,requests,created_at FROM tests ORDER BY created_at DESC`)
	if rows == nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var t Test
		rows.Scan(&t.ID, &t.Name, &t.URL, &t.Method, &t.Headers, &t.Body, &t.Concurrency, &t.Requests, &t.CreatedAt)
		d.db.QueryRow(`SELECT COUNT(*) FROM runs WHERE test_id=?`, t.ID).Scan(&t.RunCount)
		o = append(o, t)
	}
	return o
}
func (d *DB) DeleteTest(id string) error {
	d.db.Exec(`DELETE FROM runs WHERE test_id=?`, id)
	_, err := d.db.Exec(`DELETE FROM tests WHERE id=?`, id)
	return err
}
...
func (d *DB) runLoad(run *Run, t *Test) {
	...
	run.TotalRequests = successes + failures
	run.Successes = successes
	run.Failures = failures
	run.DurationMs = int(elapsed.Milliseconds())
	if len(latencies) > 0 {
		total := 0.0
		run.MinMs = latencies[0]
		run.MaxMs = latencies[0]
		...
		run.P99Ms = latencies[idx]
	}
	if elapsed.Seconds() > 0 {
		run.ReqPerSec = float64(run.TotalRequests) / elapsed.Seconds()
	}
	run.Status = "done"
	run.FinishedAt = now()
...
func (d *DB) GetRun(id string) *Run {
	var r Run
	if d.db.QueryRow(`SELECT id,test_id,status,total_requests,successes,failures,avg_ms,min_ms,max_ms,p99_ms,req_per_sec,duration_ms,started_at,finished_at FROM runs WHERE id=?`, id).Scan(&r.ID, &r.TestID, &r.Status, &r.TotalRequests, &r.Successes, &r.Failures, &r.AvgMs, &r.MinMs, &r.MaxMs, &r.P99Ms, &r.ReqPerSec, &r.DurationMs, &r.StartedAt, &r.FinishedAt) != nil {
		return nil
	}
	return &r
}
func (d *DB) ListRuns(testID string) []Run {
	rows, _ := d.db.Query(`SELECT id,test_id,status,total_requests,successes,failures,avg_ms,min_ms,max_ms,p99_ms,req_per_sec,duration_ms,started_at,finished_at FROM runs WHERE test_id=? ORDER BY started_at DESC LIMIT 20`, testID)
	if rows == nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var r Run
		rows.Scan(&r.ID, &r.TestID, &r.Status, &r.TotalRequests, &r.Successes, &r.Failures, &r.AvgMs, &r.MinMs, &r.MaxMs, &r.P99Ms, &r.ReqPerSec, &r.DurationMs, &r.StartedAt, &r.FinishedAt)
		o = append(o, r)
	}
	return o
}
